...
- **No credential handling of any kind.** This Actor's BYOK status is **none** (see the README's Cost & BYOK Disclosure section) — it calls only Tucuman's own public procurement portal, with no paid third-party API and no key of any kind to supply. There is no customer secret this Actor could leak.
- **No user-supplied code execution.** Input is a fixed JSON schema validated against `.actor/input_schema.json` (`estados`, `maxItems`, `onlyNew`, `eventTypes`, `dateRange`) — there is no arbitrary-code or arbitrary-URL input surface.
...
- **Source-portal integrity** (a compromised or spoofed Tucuman procurement-portal endpoint) is outside this Actor's control — it fetches directly from the portal's own official domain over HTTPS and does not implement independent content-signing verification beyond standard TLS.
