...
function buildListingUrl(estado: EstadoCompra, pagina: number): string {
    return `${BASE_URL}?n=1&pagina_actual=${pagina}&estado_compra=${estado}`;
}

export async function fetchTenders(estados: EstadoCompra[], maxItems: number): Promise<TenderRecord[]> {
    ...
    for (const estado of estados) {
        ...
        for (let pagina = 1; pagina <= totalPages; pagina++) {
            ...
            try {
                ...
                log.warning(
                    `estado=${estado} pagina=${pagina}: fallo tras reintentos (${message}) - deteniendo esta serie.`,
                );
                break;
            }
